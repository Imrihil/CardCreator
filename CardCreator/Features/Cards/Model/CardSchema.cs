...
namespace CardCreator.Features.Cards.Model
{
    ...
    {
        public const int ParamsNumber = 7;
        ...
        public CardSchema(string name, Image background, int widhtPx, int heightPx, double widthInch, double heightInch, IList<ElementSchema> elementSchemas, Color? defaultColor) : base(elementSchemas)
        {
            Name = name;
        ...
        public CardSchema(ILogger logger, IFontProvider fontProvider, IImageProvider imageProvider, IList<string> parameters, List<List<string>> elementSchemasParams, string directory) :
            this(
                ...
                InitElementSchemas(logger, fontProvider, imageProvider, elementSchemasParams, directory, TryGetColor(parameters[ColorIdx])),
                TryGetColor(parameters[ColorIdx])
            )
        { }

        private static IList<ElementSchema> InitElementSchemas(ILogger logger, IFontProvider fontProvider, IImageProvider imageProvider, List<List<string>> elementSchemasParams, string directory, Color? defaultColor)
        {
            var elementSchemas = new List<ElementSchema>(elementSchemasParams.Select(elementSchemaParams => new ElementSchema(logger, imageProvider, fontProvider, elementSchemaParams, directory, defaultColor)));
